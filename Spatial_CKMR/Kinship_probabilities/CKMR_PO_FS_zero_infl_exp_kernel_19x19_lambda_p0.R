...
## daily dispersal distance conditional upon movement (1 / lambda_d), and the daily  ##
## staying probability (p0) - that maximize the likelihood given the data.           ##
##                                                                                   ##
...
logLike_MOL <- function(N_A, mu_A, N_F, T_E, T_L, T_P, T_A, 
  ...
  # zero-inflated exponential kernel with staying probability, p0, and mean dispersal
  # distance conditional upon movement, 1/lambda, we have:
  
  # Initialize an empty square matrix M (numPatches x numPatches)
  ...
  for (i in 1:numPatches) {
    ...
    # Initialize denominator (normalization constant) for movement weights from patch i.
    Denominator <- 0
    
    # First loop: calculate sum of movement weights to all *other* patches (exclude self).
    for (j in 1:numPatches) {
      if (i != j) {
        # Compute weight for moving from i to j using distance decay.
        Denominator <- Denominator + exp(-lambda * distMat[i, j])
      }
    }
    
    # Second loop: assign movement probabilities to each patch j.
    for (j in 1:numPatches) {
      if (i == j) {
        # For staying in the same patch, assign fixed probability p0.
        M[i, j] <- p0
      } else {
        # For moving to other patches, distribute (1 - p0) proportionally by distance weight.
...
lambda <- 1/30
p0 <- 0.75
OptimScale <- lambda / p0
DispersalPars <- c(lambda / OptimScale, p0)

# Choose lower & upper limits of lambda & delta for optimization algorithm:
lambda_lower <- 0.001
p0_lower <- 0.01
DispersalPars_lower <- c(lambda_lower / OptimScale, p0_lower)

lambda_upper <- 1.00
p0_upper <- 0.99
DispersalPars_upper <- c(lambda_upper / OptimScale, p0_upper)
