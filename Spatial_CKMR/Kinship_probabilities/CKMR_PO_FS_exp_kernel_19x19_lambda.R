...
## daily dispersal distance (1 / lambda_d) - that maximize the likelihood given the  ##
## data.                                                                             ##
##                                                                                   ##
...
logLike_MOL <- function(N_A, mu_A, N_F, T_E, T_L, T_P, T_A, 
  ...
  # mean dispersal distance conditional upon movement, 1/lambda, we have:
  M <- matrix(rep(0, numPatches^2), nrow = numPatches, byrow = TRUE)
  # Loop over each origin patch i
  for (i in 1:numPatches) {
    # Initialize denominator for normalization (so that row probabilities sum to 1)
    Denominator <- 0
    # First, calculate the unnormalized movement weights from patch i to all patches j
    for (j in 1:numPatches) {
      # Weight is based on an exponential decay function of distance: exp(-lambda * 
      # distance)
      Denominator <- Denominator + exp(-lambda * distMat[i, j])
    }
    # Now, normalize the weights to get probabilities and fill row i of the matrix
    for (j in 1:numPatches) {
      # Calculate the normalized probability of moving from patch i to patch j
...
logLike_MOA <- function(N_A, mu_A, N_F, T_E, T_L, T_P, T_A, 
  ...
  for (i in 1:numPatches) {
    ...
    # First, calculate the unnormalized movement weights from patch i to all patches j
    for (j in 1:numPatches) {
      # Weight is based on an exponential decay function of distance: exp(-lambda * distance)
      Denominator <- Denominator + exp(-lambda * distMat[i, j])
    }
    # Now, normalize the weights to get probabilities and fill row i of the matrix
    for (j in 1:numPatches) {
...
logLike_FOL <- function(N_A, mu_A, N_F, T_E, T_L, T_P, T_A, 
  ...
  # First, calculate the daily transition matrix, which describes the probability that,
  # given a mosquito is at node i on one day, it is at node j on the next. Assuming an
  # exponential kernel with mean daily dispersal distance conditional upon movement, 
  # 1/lambda, we have:
...
# Choose initial value of lambda for the optimization algorithm:
lambda <- 1/15
DispersalPars <- lambda

# Choose lower & upper limits of lambda for optimization algorithm:
lambda_lower <- 1/100
DispersalPars_lower <- lambda_lower

lambda_upper <- 1/1
DispersalPars_upper <- lambda_upper
...
lambda_fit <- DispersalPars_fit$p1
