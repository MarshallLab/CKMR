...
#                                               /_/   /_/
###############################################################################
# mPlex simulation for 1000 adult females and 4000 larvae sampled biweekly over
# 90 days
###############################################################################
...
# Sampling for females 6.0% per week, converted to biweekly (to obtain total of
# 1000 sampled adult females):
# Sampling for larvae 0.96% per week, converted to biweekly (to obtain total of
# 4000 sampled larvae):
sampDayIndex<-ceiling(x=seq.int(from=501, to =simTime, by = 3.5)) # biweekly
sampDays[sampDayIndex ,2, ] <- TRUE
sampDays[sampDayIndex ,3, ] <- TRUE
sampDays[sampDayIndex ,4, ] <- TRUE
sampDays[sampDayIndex ,5, ] <- TRUE
...
for(i in 1:numPC){
  ...
  ####################
  readFiles <- c('/000_L.csv','/000_F.csv','/000_M.csv')
  writeFiles <- c('/cut_L.csv','/cut_F.csv','/cut_M.csv')
